...
export class ConsecutiveComponent implements OnInit, OnDestroy {
  ...
  */
  constructor(
    ...
    _globalService.getConsecutiveState().subscribe((ConsecutiveState) => {
      this.list_consecutive_state = ConsecutiveState;
    });

    //Get the consecutive types
    _globalService.getConsecutiveTypes().subscribe((ConsecutiveTypes) => {
    ...
    _settingsService.getConsecutive().subscribe((Consecutive) => {
  ...
  */
  loadDetail(consecutive) {
    ...
    this.detail_consecutive_valid_since =
      consecutive.vigencia_desde_consecutivo_reporte;
    this.detail_consecutive_valid_until =
      consecutive.vigencia_hasta_consecutivo_reporte;
    ...
    this.detail_consecutive_remaining =
      consecutive.consecutivo_restante_reporte;
    this.detail_consecutive_state_id =
      consecutive.id_estado_consecutivo_reporte;
    this.detail_consecutive_state =
    ...
    if (consecutive.year_consecutivo_reporte == year) {
      if (
        Number(consecutive.consecutivo_actual_reporte) <=
        Number(consecutive.consecutivo_hasta_reporte)
      ) {
  ...
  */
  get rc() {
    return this.consecutive_form.controls;
  }
  ...
  registerConsecutive() {
    ...
    //Form values
    const consecutive_current = this.consecutive_form.get("consecutive_current")
      .value;
    const consecutive_faculty = this.consecutive_form.get("consecutive_faculty")
      .value;
    const consecutive_type = this.consecutive_form.get("consecutive_type")
      .value;
  ...
  */
  get uc() {
    return this.update_consecutive_form.controls;
  }
  ...
  updateConsecutive() {
    ...
    const consecutive_state = this.update_consecutive_form.get(
      "consecutive_state"
    ).value;

    this._settingsService
      .updateConsecutive(consecutive_id, consecutive_state)
      .subscribe((data) => {
  ...
  */
  refreshData() {
    //Get the consecutives
    this._settingsService.getConsecutive().subscribe((Consecutive) => {
      const ELEMENT_DATA = Consecutive;
      //Get the elements
      this.list_consecutive_aux = new MatTableDataSource(ELEMENT_DATA);
      this.list_consecutive_aux.paginator = this.paginator;
